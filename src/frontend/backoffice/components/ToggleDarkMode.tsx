...
const BACKOFFICE_THEME = 'backoffice-theme';
...
export function ToggleDarkMode() {
  ...
  useEffect(() => {
    const savedTheme = localStorage.getItem(BACKOFFICE_THEME);
  ...
  const toggleDarkMode = () => {
    ...
    if (newDarkMode) {
      document.documentElement.setAttribute('data-theme', 'dark');
      localStorage.setItem(BACKOFFICE_THEME, 'dark');
    } else {
      document.documentElement.setAttribute('data-theme', 'light');
      localStorage.setItem(BACKOFFICE_THEME, 'light');
    }
  };
